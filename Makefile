...
otel_launcher_url := https://github.com/lightstep/otel-launcher-java/releases/download/0.14.0/lightstep-opentelemetry-javaagent-0.14.0.jar

build: lightstep-opentelemetry-javaagent-0.14.0.jar
	$(MAKE) -C server
	$(MAKE) -C client

lightstep-opentelemetry-javaagent-0.14.0.jar:
	curl -L -O ${otel_launcher_url}

run-server: build
	java -javaagent:lightstep-opentelemetry-javaagent-0.14.0.jar  \
